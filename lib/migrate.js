...
const reverseSequelizeColType = function(col)
{
    ...
    if (attrObj instanceof Sequelize.DOUBLE)
    {
        ...
        return 'DOUBLE('+options.length.toLowerCase()+')';
    }
...
const parseIndex = function(idx) 
{
    ...
    if (idx.method)
        options.using = idx.method; // Set a type for the index, e.g. BTREE. See the documentation of the used dialect
...
const parseUniqueKey = function(tableName, indexName, idx)
{
    delete idx.customIndex;
    delete idx.column;
    delete idx.msg;
    ...
    if (typeof indexName !== 'string') {
        indexName = `uniq_${tableName}_${idx.fields.join('_')}`;
    }
    ...
    for (let model in models)
    {
        ...
        for (let column in attributes)
        {
            ...
            for(let property in attributes[column]) 
            {
                ...
                if (property === 'unique')
                {
                    if (typeof attributes[column][property] === 'string') {
                        delete attributes[column][property];
                        continue;
                    }
                }
            ...
            let prefix = 'Sequelize.';
            // NO virtual types in migration
            if (seqType === 'VIRTUAL')
            {
                log(`[SKIP] Skip VIRTUAL column "${column}"", defined in model "${model}"`);
                delete attributes[column];
                continue;
            }
        ...
        {
            let idx_out = models[model].options.indexes;
            for (let _i in models[model].uniqueKeys) {
                if (models[model].uniqueKeys[_i].customIndex) {
                    let index = parseUniqueKey(models[model].tableName, _i, models[model].uniqueKeys[_i]);
                    idx_out[index.hash + ''] = index;
                    delete index.hash;
...
let res = `{ fn: "addIndex", params: [
    ...
    ${JSON.stringify(action.options)}
                ...
                let nameOrAttrs = (action.options && action.options.name && action.options.name != '') ? `"${action.options.name}"` : JSON.stringify(action.fields);
